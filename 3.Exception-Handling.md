...
このドキュメントでは、NestJSフレームワークを使用したアプリケーションにおける例外処理の動作原理について説明します。`throw new`で発生させた例外がどのようにHTTPレスポンスに変換されるか、その詳細なメカニズムを解説します。
...
すべてのカスタム例外は`BaseException`を継承し、`statusCode`と`code`を持ちます。
...
アプリケーション起動時に`main.ts`でグローバルフィルターが登録されます：
...
この設定により、アプリケーション全体で発生するすべての例外が自動的にフィルターで処理されます。
...
- コントローラー、サービス、ミドルウェアなど、任意の層で`throw`された例外がNestJSフレームワークによって自動的に検知されます
- 処理されていない例外は、グローバル例外フィルターに自動的に転送されます

### ステップ3: HttpExceptionFilterが例外を処理

フィルターは以下のロジックで例外を処理します：
    ...
    examples: {
      badRequest: {
        ...
        value: {
          ...
          message: 'X-Site-Identifierヘッダーが必要です',
          status: 400,
        },
      },
      accessDenied: {
        ...
        value: {
          code: 'ACCESS_DENIED',
          message: 'サービス一覧へのアクセスが拒否されました',
          status: 403,
          details: {
            resource: 'サービス一覧',
            reason:
              'サイトの権限設定により、このサービス一覧にアクセスできません',
          },
        },
      },
      validationError: {
        ...
        value: {
          ...
          status: 422,
          details: {
            page: 'ページ番号は1以上の整数である必要があります',
            per_page: '1ページあたりの件数は1-100の範囲で指定してください',
            q: 'キーワードは100文字以内で入力してください',
          },
        },
      },
      serverError: {
...
- 開発者は例外をスローするだけで、HTTPレスポンスの構築を意識する必要がない
...
// ❌ 悪い例：一般的なErrorを使用
throw new Error('Invalid member ID');  // これは500エラーになる
```
...
```typescript
  ...
  // 未知のエラーの場合はInternalServerErrorExceptionにラップ
  throw new InternalServerErrorException(
...
この仕組みにより、アプリケーション全体で一貫したエラーハンドリングが実現され、フロントエンドとバックエンドの間で明確なエラー契約が確立されます。
